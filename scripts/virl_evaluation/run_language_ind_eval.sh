...
export PYTHONPATH=/path/RL_Heals_SFTL:$PYTHONPATH
...
DS_SKIP_CUDA_CHECK=1 accelerate launch \
    ...
    -f /path/RL_Heals_SFT/evaluation/configs/llama_virl_language.yaml \
    ...
    --num_traj=${NUM_TRAJ} \
 2>&1 | tee eval-ind-llama-${today}-${check_point}.log